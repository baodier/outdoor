...
function ajaxinnerhtml(showid, s) {
	...
	//5jj
	if(showid==null) showid='ayaya_temp48568231';
...
function showPrompt(ctrlid, evt, msg, timeout,type) {
	...
	if(ctrlid) {
		...
		//if(type!='loading')	$$(menuid).style.top = 88 + 'px';
	}
}
...
function showWindow(k, url, mode, cache, menuv) {
	...
	if(!menuObj) {
		...
		menuObj.innerHTML = '<table cellpadding="0" cellspacing="0" class="fwin"><tr><td class="t_l"></td><td class="t_c" ondblclick="hideWindow(\'' + k + '\')"></td><td class="t_r"></td></tr><tr><td class="m_l" ondblclick="hideWindow(\'' + k + '\')"><span></span></td><td class="m_c">'+$open+'<div id="fwin_content_' + k + '"></div></td><td class="m_r" ondblclick="hideWindow(\'' + k + '\')"></td></tr><tr><td class="b_l"></td><td class="b_c" ondblclick="hideWindow(\'' + k + '\')"></td><td class="b_r"></td></tr></table>';
		if (mode == 'html') {
...
function showmessage(s,type,c) {
	...
	if(type=='succeed'){
		...
		showPrompt(null, null, '<div class="msgbox msgbox_succeed"><div class="msgbox_layer"><span class="msgbox_layer_l"></span><span class="msgbox_layer_img"></span>' + s + '<span class="msgbox_layer_r"></span></div></div>', 0);
	}else if(type=='loading'){
		
		
		showPrompt(null, null, '<div class="msgbox msgbox_loading"><div class="msgbox_layer"><span class="msgbox_layer_l"></span><span class="msgbox_layer_img"></span>' + s + '<span class="msgbox_layer_r"></span></div></div>', 0,'loading');
	}else{
		
		showPrompt(null, null, '<div class="msgbox msgbox_error"><div class="msgbox_layer"><span class="msgbox_layer_l"></span><span class="msgbox_layer_img"></span>' + s + '<span class="msgbox_layer_r"></span></div></div>', 0);
	}
	
	if(type!='loading'){
		
		setTimeout(function () {hidemask();}, 1400);
		hidetimer=setTimeout(function () {hideMenu(1, 'prompt');$$('append_parent').removeChild($$('ntcwin'));}, 1600);
...
function hidemessage() {
	setTimeout(function () {hidemask();}, 200);
	setTimeout(function () {hideMenu(1, 'prompt');$$('append_parent').removeChild($$('ntcwin'));}, 200);
}

function showloading(msg) {
		msg = isUndefined(msg) ? '请稍候' : msg;
...
function hidewin(){
	hidemask(1);
	setTimeout(function () {$$('append_parent').removeChild($$('fwin_xxx'));}, 0);
}
function ajaxp(id){
	ajaxpost(id,'','','');
	return false;
...
{
	if(!id) id='captcha';
	var _z=Math.floor(Math.random()*10)+Math.floor(Math.random()*10)+Math.floor(Math.random()*10);
	document.getElementById(id+'img').src=ROOTPATH+'static/captcha.php?'+_z;
...
function showmask(c){
	...
	if(isUndefined(c)) c=0;
        ...
        tMask=setInterval(function(){
			
			if(o.filters){
				if(c) {
					o.filters[0].opacity=opa;
				return;	
				}
                if(o.filters[0].opacity>opa)
                {
                        clearInterval(tMask);
                        return;
                }
                o.filters[0].opacity=o.filters[0].opacity+8;
			}else{
				if(c) {
				...
				if(!o.style.opacity) o.style.opacity=0;				
				 if(o.style.opacity>(opa/100))
                {
                        clearInterval(tMask);
                        return;
                }
				 o.style.opacity=parseFloat(o.style.opacity)+0.1;
			}
        },1);
}
...
function hidemask(f){        
		...
		if(isUndefined(f) && $$('fwin_xxx')) return;
        ...
        tMask=setInterval(function(){
			...
			if(o.filters){
                ...
                o.filters[0].opacity=o.filters[0].opacity-15;
			}else{
				
				if(!o.style.opacity) o.style.opacity=opa/100;	
				 if(o.style.opacity<=0 )
                {
				 ...
				 o.style.opacity=parseFloat(o.style.opacity).toFixed(2)-0.038;
			}
        },1);
}
...
function showselect(obj, inpid, t) {
	if(!obj.id) {
		var t = !t ? 0 : t;

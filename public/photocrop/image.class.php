<?php
	class Images{
		...
		function __construct($inputName,$allowType="",$allowSize="",$saveDir="",$isRename=true) {
			...
			$this->saveDir = $_SERVER['DOCUMENT_ROOT'].$this->saveDir;
		}	//end __construct
		...
		function move_uploaded(){        //移动上传文件
			...
			if($result){
				...
				$patharr = explode($_SERVER['DOCUMENT_ROOT'],$path);
		 ...
		 * @return void
		 +----------------------------------------------------------
		 */		 
		function thumb($image,$from,$is_save=true,$suofang=0,$type='',$maxWidth=300,$maxHeight=300,$interlace=true){
			// 获取原图信息
		    ...
		    $info  = $this->getImageInfo($image);
			 if($info !== false) {
				...
				if ($suofang==1) {
					...
					$thumbname01 = substr_replace($image,"01.".$type,$length);		//大尺寸头像
					$thumbname02 = substr_replace($image,"02.".$type,$length);		//中尺寸头像
					$thumbname03 = substr_replace($image,"03.".$type,$length);		//小尺寸头像
					$imageFun($thumbImg,$thumbname01,100);
					$imageFun($thumbImg,$thumbname02,100);
					$imageFun($thumbImg,$thumbname03,100);
					
					if($from == 'group' )
						$ava_size = array('big'=>180,'middle'=>90,'small'=>50);
					elseif($from =='person')
						$ava_size = array('big'=>140,'middle'=>50,'small'=>30);
					

					$thumbImg01 = imagecreatetruecolor($ava_size['big'],$ava_size['big']);
					imagecopyresampled($thumbImg01,$thumbImg,0,0,$_POST['x'],$_POST['y'],$ava_size['big'],$ava_size['big'],$_POST['w'],$_POST['h']);

					$thumbImg02 = imagecreatetruecolor($ava_size['middle'],$ava_size['middle']);
					imagecopyresampled($thumbImg02,$thumbImg,0,0,$_POST['x'],$_POST['y'],$ava_size['middle'],$ava_size['middle'],$_POST['w'],$_POST['h']);
					
					$thumbImg03 = imagecreatetruecolor($ava_size['small'],$ava_size['small']);
					imagecopyresampled($thumbImg03,$thumbImg,0,0,$_POST['x'],$_POST['y'],$ava_size['small'],$ava_size['small'],$_POST['w'],$_POST['h']);
					...
					//返回包含大中小头像路径的数组
					return array('big' => $thumbname01 , 'middle' =>$thumbname02, 'small' => $thumbname03);	
				}else{
					...
					if($is_save == false){
						...
						$patharr = explode($_SERVER['DOCUMENT_ROOT'],$thumbname03);
						return $patharr[1];
					}
				}
			 }
...
//	./uploads/2011/6/23/201106231015231234985801.jpg	----->裁剪的大头像  180*180
//	./uploads/2011/6/23/201106231015231234985802.jpg	----->裁剪的小头像	50*50
//	./uploads/2011/6/23/201106231015231234985803.jpg	----->裁剪的小头像	30*30
//	./uploads/2011/6/23/201106231015231234985804.jpg	----->超过规定尺寸后的缩略图